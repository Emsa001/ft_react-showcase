...
const updateBoolean = (
    ...
    if (newNode === false) {
        unMountNode(oldNode);
        if (
            typeof oldNode != "object" || (typeof oldNode === "object" && typeof (oldNode as ReactElement).type !== "function")
        ) {
            ref?.remove();
        }
        return;
...
const updateDifferentTypes = (oldNode: ReactElement, newNode: ReactElement, ref: Element | null, name: string) => {
    if (typeof oldNode === typeof newNode) false;
...
const updateFunctionComponent = (oldNode: ReactElement, newNode: ReactElement, ref: Element | null) => {
    const oldComponent = React.components.get(oldNode.componentName!);
    if (!oldComponent) 
        return console.warn("Old component not found", oldNode, newNode);
    ...
    const newComponent = (newNode.type as ComponentType)(newNode.props, ...newNode.children);
...
const updateDifferentObjectTypes = (oldNode: ReactElement, newNode: ReactElement, ref: Element | null, name: string) => {
...
const updateElement = (oldNode: ReactElement, newNode: ReactElement, ref: Element | null, name: string) => {
    const oldProps = oldNode.props || {};
    const newProps = newNode.props || {};
...
export function update(props: UpdateProps) {
    ...
    // Step 6
    if(updateElement(oldNode, newNode, ref, name)) return;
